...
void AccelSetup() {
  ...
  I2cReadByte(1, MMA7660_REG_MODE, &modeReg);
  
  modeReg &= ~MMA7660_MODE_TON;
  modeReg &= ~MMA7660_MODE_MODE;

  I2cWriteByte(1, MMA7660_REG_MODE, modeReg);

  I2cWriteByte(1, MMA7660_REG_SR, MMA7660_AUTOSLEEP_64);
...
void AccelGetValue(float* x, float* y, float* z) {
  ...
  do {
    I2cReadByte(1, MMA7660_REG_XOUT, &valueX);
  } while (valueX & 0x40); // check alert bit
  
  do {
    I2cReadByte(1, MMA7660_REG_YOUT, &valueY);
  } while (valueY & 0x40); // check alert bit
  
  do {
    I2cReadByte(1, MMA7660_REG_ZOUT, &valueZ);
